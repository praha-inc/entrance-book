...
※ PrAhaの開発スタイルやチームの雰囲気を実際に感じていただくために、まずはお試し入社として業務委託契約のご案内も可能です（1か月の期間限定契約・最大工数30時間）
...
- [PrAha Challenge](https://praha-challenge.com)のメンター

### 具体的な仕事内容の一例
- ユーザーストーリーの実装
- ドメインエキスパートへのヒアリングなどを通して事業理解を深め、適切なモデリングやDBスキーマ設計をする
...
- フロントエンド
  - Next.js
  - Vanilla Extract（CSS Modulesに移行中⋯）
  - Radix UI（プラハ製のOSSを製作予定⋯）
  - Storybook
  - Flutter
  - Chromatic(VRT)
- バックエンド
...
1. カジュアル面談(30~60分)
2. 技術面談(下記2つの試験を実施します)
   - フロントエンド技術面談(60~120分)
   - バックエンド技術面談(60~120分)

フロントエンドとバックエンドの技術面談は、お好きな順番で受けていただけます。もし「バックエンドを先に受けたい」というご希望があれば、カジュアル面談の際にお伝えください（特にご指定がなければフロントエンドからご案内いたします）。
...
### 1. カジュアル面談
ワイワイ雑談しましょう！　PrAhaのこと、働き方、選考のことなど、気になることがあれば何でも聞いてください！
「応募するかはまだ迷ってるけど、ちょっと話を聞いてみたい」ぐらいでも大丈夫です。気軽に面談をお申し込みください！

### 2. 技術面接
フロントエンドとバックエンドの技術面接を実施します。どちらもライブコーディング形式です。
＊技術面接をご希望いただいた場合でも、必須スキルや働き方との相性を踏まえて、実施を見送る場合があります。

#### なぜフロントエンドとバックエンドの両方を面接するのか
PrAhaの開発では、1人のエンジニアがユースケース単位でフロントエンドからバックエンドまで一気通貫で実装することが多いです。
...
#### なぜライブコーディングなのか
創業初期は口頭で技術面接をしていたのですが、口頭面接で測れるのはあくまで受け答えの上手さだけで、コードを書くのは上手だけど受け答えが苦手な方が評価されないなど、面接としての不完全さが目立つようになりました。

どれぐらいコードを書けるのか判断するには、実際コードを書いている様子を見せていただくのが最も効果的だと考えたため、今のライブコーディング面接に落ち着きました。

#### 共通の評価ポイント
フロントエンド・バックエンドの両方の面接で、以下を評価します。
...
2. 架空のサービスを想定したエンドポイントの実装
    - 1つのユースケースを実装していただきます。
    - なお、言語はNode.js+TypeScript、またはPHPでお願いしています。
